    ...
    public function getAvailableBooks ()
    {
        try {
            ...
            while ($row = $this->stat->fetch(PDO::FETCH_ASSOC)) {
                $publisher = new Éditeurs($row['publisher_name'], $row['publisher_id']);
                new Book(
                    $row['book_id'],
                    $row['title'],
                    $row['ISBN'],
...
");
            ...
            return new Book(
                ...
                $row['quantity'],
                new Author($row['author'], ''),
                new Éditeurs($row['publisher_name'])
            );
        } catch (PDOException $exception) {
            echo $exception->getMessage();
            die();
...
/// they do not test
    ...
    public function getAllÉditeurs (): PDOStatement
    {
        try {
            $this->stat = $this->conn->prepare("
    ...
    public function getPublisher ($id): Éditeurs
    {
        try {
            $this->stat = $this->conn->prepare("
            ...
            $this->stat->execute();
            $row = $this->stat->fetch(PDO::FETCH_ASSOC);
            return new Éditeurs($row['name'], $row['publisher_id']);
        } catch (PDOException $exception) {
            echo $exception->getMessage();
            return NULL;
        }
    ...
    public function getPublisherbyname ($name): Éditeurs
    {
        try {
            $this->stat = $this->conn->prepare("
